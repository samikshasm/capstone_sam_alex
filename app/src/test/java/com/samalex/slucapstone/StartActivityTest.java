...
public class StartActivityTest {
    ...
    public void testCalculateUserStartTimeWhenUserLoginBeforeMidnight() throws Exception {
        ...
        long actualMillis = startActivity.calculateUserStartTime(mockCurrentDateTimeCalendar, mockCycleLength, mockCycleOffset, 1);
    ...
    @Test
    public void testCalculateUserStartTimeWhenUserLoginBeforeMidnightStartFromCycle3() throws Exception {
        StartActivity startActivity = new StartActivity();
        ...
        // expect start time to be 03/19/2019 5:00:00.000 AM
        Calendar expected = (Calendar) mockCurrentDateTimeCalendar.clone();
        expected.set(Calendar.HOUR_OF_DAY, 5);
        expected.add(Calendar.DAY_OF_MONTH, -2);

        long actualMillis = startActivity.calculateUserStartTime(mockCurrentDateTimeCalendar, mockCycleLength, mockCycleOffset, 3);

        assertEquals(expected.getTimeInMillis(), actualMillis);
    }

    @Test // when the user login time is between midnight and CYCLE_OFFSET
    public void testCalculateUserStartTimeWhenUserLoginBetweenMidnightAndCycleOffsetStartFromCycle3() throws Exception {
        StartActivity startActivity = new StartActivity();
        ...
        // expect start time to be 03/18/2019 5:00:00.000 AM
        Calendar expected = (Calendar) mockCurrentDateTimeCalendar.clone();
